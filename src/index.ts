...
// Response: PokemonSet[]
// ---------------------------------------------------------------------------
app.post("/api/v1/parse", (req: Request, res: Response): any => {
...
// ---------------------------------------------------------------------------
app.post("/api/v1/scrape", async (req: Request, res: Response): Promise<any> => {
  try {
    const { team } = req.body;
...
// ---------------------------------------------------------------------------
app.post("/api/v1/analyze", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const resolvedApiKey: string | undefined = bodyApiKey || process.env.GOOGLE_GENAI_API_KEY;
    if (!resolvedApiKey) {
    ...
    try {
      const report = await generateVGCStrategyReport(
        parsedTeam,
        scrapedData,
        resolvedApiKey,
